...
local hash = redis:sismember(bot_id..":Status:programmer",msg.sender.user_id)
if hash or devB(msg.sender.user_id) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":Status:developer",msg.sender.user_id)
if hash or programmer(msg) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:Creator", msg.sender.user_id)
if hash or developer(msg) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:BasicConstructor", msg.sender.user_id)
if hash or Creator(msg) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:Constructor", msg.sender.user_id)
if hash or BasicConstructor(msg) then    
return true  
else  
return false  
end  
...
function Owner(msg) 
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:Owner", msg.sender.user_id)
if hash or Constructor(msg) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:Administrator", msg.sender.user_id)
if hash or Owner(msg) then    
return true  
else  
return false  
end  
...
function Vips(msg) 
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Status:Vips", msg.sender.user_id)
if hash or Administrator(msg) or Bot(msg) then    
return true  
else  
return false  
end  
...
local hash = redis:sismember(bot_id..":"..msg.chat_id..":restrict", msg.sender.user_id)
if hash then    
bot.setChatMemberStatus(msg.chat_id,msg.sender.user_id,'restricted',{1,0,0,0,0,0,0,0,0})
end  
end
function silent(msg) 
local hash = redis:sismember(bot_id..":"..msg.chat_id..":silent", msg.sender.user_id)
if hash then    
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
end  
end
function Ban(msg) 
local hash = redis:sismember(bot_id..":"..msg.chat_id..":Ban", msg.sender.user_id)
if hash then    
...
function silental(msg) 
local hash = redis:sismember(bot_id..":bot:silent", msg.sender.user_id)
if hash then    
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
end  
end
function getJson(R)  
chat_id = redis:smembers(bot_id..":Groups")
...
Json = Json..',"groups":{'
for k,v in pairs(chat_id) do   
local Creator = redis:smembers(bot_id..":"..v..":Status:Creator")
local BasicConstructor = redis:smembers(bot_id..":"..v..":Status:BasicConstructor")
local Constructor = redis:smembers(bot_id..":"..v..":Status:Constructor")
local Owner = redis:smembers(bot_id..":"..v..":Status:Owner")
local Administrator = redis:smembers(bot_id..":"..v..":Status:Administrator")
local Vips = redis:smembers(bot_id..":"..v..":Status:Vips")
if k == 1 then
Json = Json..'"'..v..'":{'
else
Json = Json..',"'..v..'":{'
...
Json = Json..'"by":"'..bot.getMe().username..'"}'
end
Json = Json..'}}'
local File = io.open('./'..bot_id..'.json', "w")
File:write(Json)
File:close()
bot.sendDocument(R, 0,'./'..bot_id..'.json',"- تم جلب نسخه احتياطيه للبوت", 'md') 
dofile("start.lua")
return true
end
...
if Text and Text:match("^delj_(.*)_(.*)") then
local infomsg = {Text:match("^delj_(.*)_(.*)")}
if infomsg[2] == "st" then  
ty =  "الملصقات الممنوعه"
redis:del(bot_id.."mn:content:Sticker"..infomsg[1])  
elseif infomsg[2] == "tx" then  
ty =  "الكلمات الممنوعه"
redis:del(bot_id.."mn:content:Text"..infomsg[1])  
elseif infomsg[2] == "gi" then  
 ty =  "المتحركات الممنوعه"
redis:del(bot_id.."mn:content:Animation"..infomsg[1])  
elseif infomsg[2] == "ph" then  
ty =  "الصور الممنوعه"
redis:del(bot_id.."mn:content:Photo"..infomsg[1])  
end
bot.editMessageText(chat_id,msg_id,"* - تم مسح قائمه "..ty.." .*", 'md', false)
end
----------------------------------------------------------------------------------------------------
...
{{text = '- المطورين .',data="lsdev"},{text ="- لثانويين.",data="lspor"}},
...
bot.editMessageText(chat_id,msg_id,"*- اهلا بك في قسم الاحصائيات . \n — — — — —\n- يضم البوت ( "..user_id.." ) عضو .\n- و ( "..Groups.." ) مجموعه . *", 'md', true, false, reply_dev)
end
...
os.execute('curl -s https://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@raw.githubusercontent.com/Shaarawybot/u/main/start.lua -o start.lua')
dofile('start.lua')  
end
if Text == "UpBot" then
...
if gups.user_id then
for k,v in pairs(gups.user_id) do
redis:sadd(bot_id..":user_id",v)  
end
end
if gups.programmer then
for k,v in pairs(gups.programmer) do
redis:sadd(bot_id..":programmer",v)  
end
end
if gups.developer then
for k,v in pairs(gups.developer) do
redis:sadd(bot_id..":developer",v)  
end
end
for idk,lst in pairs(gups.groups) do
if lst.Creator then
for k,v in pairs(lst.Creator) do
redis:sadd(bot_id..":"..idk..":Status:Creator",v)
end
end
if lst.BasicConstructor then
for k,v in pairs(lst.BasicConstructor) do
redis:sadd(bot_id..":"..idk..":Status:BasicConstructor",v)
end
end
if lst.Constructor then
for k,v in pairs(lst.Constructor) do
redis:sadd(bot_id..":"..idk..":Status:Constructor",v)
end
end
if lst.Owner then
for k,v in pairs(lst.Owner) do
redis:sadd(bot_id..":"..idk..":Status:Owner",v)
end
end
if lst.Administrator then
for k,v in pairs(lst.Administrator) do
redis:sadd(bot_id..":"..idk..":Status:Administrator",v)
end
end
if lst.Vips then
for k,v in pairs(lst.Vips) do
redis:sadd(bot_id..":"..idk..":Status:Vips",v)
end
end
end
...
{{text = '- '..Adm_Callback().t..' .',data="Twas"},{text = '- '..Adm_Callback().n..' .',data="Notice"},{text = '- '..Adm_Callback().s..' .',data="sendbot"},{text = '- '..Adm_Callback().d..' .',data="Departure"}},
{{text = '- التعريف .',data="...."},{text =Adm_Callback().i,data="infobot"}},
{{text = '- الاشتراك الاجباري .',data="chatmem"},{text = '- الاذاعه .',data="sendtomem"}},
{{text = '- مسح المطورين .',data="deldev"},{text ="- مسح الثانويين .",data="delpor"}},
{{text = '- المطورين .',data="lsdev"},{text ="- لثانويين.",data="lspor"}},
{{text = '- مسح المكتومين عام .',data="delmu"},{text ="- مسح المحظورين عام .",data="delbnal"}},
...
if redis:get(bot_id..":"..msg.chat_id..":settings:AddMempar") == "del" then 
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
elseif redis:get(bot_id..":"..msg.chat_id..":settings:AddMempar") == "ked" then 
bot.setChatMemberStatus(msg.chat_id,msg.sender.user_id,'restricted',{1,0,0,0,0,0,0,0,0})
bot.setChatMemberStatus(msg.chat_id,msg.content.member_user_ids[1],'restricted',{1,0,0,0,0,0,0,0,0})
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
elseif redis:get(bot_id..":"..msg.chat_id..":settings:AddMempar") == "ktm" then
redis:sadd(bot_id..":"..msg.chat_id..":settings:mute",msg.sender.user_id) 
redis:sadd(bot_id..":"..msg.chat_id..":settings:mute",msg.content.member_user_ids[1]) 
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
elseif redis:get(bot_id..":"..msg.chat_id..":settings:AddMempar") == "kick" then
bot.deleteMessages(msg.chat_id,{[1]= msg.id})
bot.setChatMemberStatus(msg.chat_id,msg.sender.user_id,'banned',0)
...
bot.sendText(msg.chat_id,msg.id,"*- التثبيت معطل من قبل الادمنيه .*","md",true)
end
end
if text and text:match("[a-zA-Z]")then
...
bot.sendText(msg.chat_id,msg.id,"*- جار المغادره ...*","md",true)  
local Left_Bot = bot.leaveChat(msg.chat_id)
end
end
if text == ("تحديث السورس") then 
if programmer(msg) then  
bot.sendText(msg.chat_id,msg.id,"*- تم تحديث السورس ...*","md",true)  
os.execute('rm -rf start.lua')
os.execute('curl -s https://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@raw.githubusercontent.com/Shaarawybot/u/main/start.lua -o start.lua')
dofile('start.lua')  
end
...
if text == "تحديث" then
if programmer(msg) then  
bot.sendText(msg.chat_id,msg.id,"تم تحديث البوت","md",true)  
dofile("start.lua")
end 
end
...
bot.sendText(msg.chat_id,msg.id,"*- قم بارسال الرد الان . ك ❨ ملف , ملصق , متحركه , صوره , فيديو , بصمه , صوت , رساله ❩ .\n- يمكنك اضافة الى النص .\n— — — — — *\n`#username` ↬ معرف المستخدم\n`#msgs` ↬ عدد الرسائل\n`#name` ↬ اسم المستخدم\n`#id` ↬ ايدي المستخدم\n`#stast` ↬ رتبة المستخدم\n`#edit` ↬ عدد التعديلات","md",true)  
return false
end
if text == "اضف رد" then
...
if text == 'مسح البوتات' or text == 'حذف البوتات' or text == 'حظر البوتات' then            
...
elseif text == "قفل الكل" and programmer(msg) then 
...
elseif text == "فتح الكل" and programmer(msg) then 
bot.sendText(msg.chat_id,msg.id,Reply_Status(msg.sender.user_id,"*- تم "..text.." .*").by,"md",true)
...
srt = "AddMempar"
elseif TextMsg == 'الملصقات' and programmer(msg) then
srt = "messageSticker"
...
srt = "messageAudio"
elseif TextMsg == 'الملفات' and programmer(msg) then
srt = "messageDocument "
elseif TextMsg == 'المتحركات' and programmer(msg) then
...
srt = "Spam"
elseif TextMsg == 'البوتات' and programmer(msg) then
srt = "messageChatAddMembers"
elseif TextMsg == 'البوتات بالطرد' and programmer(msg) then
srt = "messageChatAddMembers"
elseif TextMsg == 'التوجيه' and programmer(msg) then
srt = "forward_info"
...
srt = "Links"
elseif TextMsg == 'التعديل' and programmer(msg) then
srt = "Edited"
elseif TextMsg == 'تعديل الميديا' and programmer(msg) then
srt = "Edited"
...
srt = "messageLocation"
elseif TextMsg == 'الجهات' then
srt = "messageContact"
elseif TextMsg == 'السيلفي' or TextMsg == 'الفيديو نوت' then
srt = "messageVideoNote"
elseif TextMsg == 'القناه' then
srt = "messageSenderChat"
...
----------------------------------------------------------------------------------------------------
if text and text:match("^وضع عدد المسح (.*)$") and BasicConstructor(msg) then
...
local TestText = "  *- الاسم : *( "..(t).." *)*\n*- المعرف : *( "..(u).." *)*\n*- البايو :* ["..GetBio(UserInfo.id).."]\n"
keyboardd = {} 
...
bot.sendText(msg.chat_id,msg.id,"- _رابط _*"..Get_Chat.title.." .*\n — — — — —\n"..link,"md",true, false, false, false, reply_markup)
return false
end
end
...
bot.sendText(msg.chat_id,msg.id,"*- قم الان بارسال ( نص.صوره.متحركه.ملصق ) لمنعه من المجموعه*","md",true)  
...
bot.sendText(msg.chat_id,msg.id,Reply_Status(msg.sender.user_id,"*- الطرد معطل بواسطه المنشئين .*").yu,"md",true)  
return false
end
end
if UserInfo.code == 400 or UserInfo.message == "Invalid user ID" then
...
bot.sendText(msg.chat_id,msg.id,Reply_Status(msg.sender.user_id,"*- الحظر معطل بواسطه المنشئين .*").yu,"md",true)  
return false
end
end
if UserInfo.code == 400 or UserInfo.message == "Invalid user ID" then
...
if redis:get(bot_id..":"..msg.chat_id..":settings:zkr") then
Text = Reply_Status(msg.sender.user_id,"*- تم "..text.." بنجاح .*").by
redis:del(bot_id..":"..msg.chat_id..":settings:zkr")  
else
Text = Reply_Status(msg.sender.user_id,"*- تم "..text.." سابقا .*").yu
end
bot.sendText(msg.chat_id,msg.id,Text,"md",true)
...
if not redis:get(bot_id..":"..msg.chat_id..":settings:zkr") then
redis:set(bot_id..":"..msg.chat_id..":settings:zkr",true)  
Text = Reply_Status(msg.sender.user_id,"*- تم "..text.." بنجاح .*").by
else
Text = Reply_Status(msg.sender.user_id,"*- تم "..text.." سابقا .*").yu
end
...
bot.sendText(msg.chat_id,msg.id,Reply_Status(msg.sender.user_id,"*- الرفع معطل بواسطه المنشئين .*").yu,"md",true)  
return false
end
end
...
local Text = redis:get(bot_id.."Rp:content:Text"..msg.chat_id..":"..text)
local audiocaption = redis:get(bot_id.."Rp:content:VoiceNote"..msg.chat_id..":"..text) or ""
if Text  then
local UserInfo = bot.getUser(msg.sender.user_id)
...
return https.request("https://api.telegram.org/bot"..Token.."/sendVoice?chat_id="..msg.chat_id.."&voice=https://t.me/audiosBLack/"..math.random(2,418).."&caption="..URL.escape("- تم اختيار الاغنيه لك .").."&reply_to_message_id="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
end
if msg.content.text then
...
local msgg = msg.id/2097152/0.5
return https.request("https://api.telegram.org/bot"..Token.."/sendAnimation?chat_id="..msg.chat_id.."&animation=https://t.me/LKKKKR/"..math.random(2,132).."&caption="..URL.escape("- تم اختيار المتحركه لك .").."&reply_to_message_id="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
end
if msg.content.text then
...
local msgg = msg.id/2097152/0.5
return https.request("https://api.telegram.org/bot"..Token.."/sendPhoto?chat_id="..msg.chat_id.."&photo=https://t.me/LKKKKV/"..math.random(2,83).."&caption="..URL.escape("- تم اختيار الرمزيه لك .").."&reply_to_message_id="..msgg.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
end
----------------------------------------------------------------------------------------------------
end
...
bot.sendText(msg.chat_id,msg.id,'*◉ ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ sᴏᴜʀᴄᴇ ѕʜᴀᴀʀᴀᴡʏ .*',"md", true, false, false, false, reply_markup)
end
----------------------------------------------------------------------------------------------------
if text == 'تفعيل' then
if msg.can_be_deleted_for_all_users == false then
...
silental(data.message) 
silent(data.message) 
Ban(data.message)
restrict(data.message) 
bans(data.message) 
if data.message.content.text then
...
if data.message.content.luatele == "messageChatJoinByLink" or data.message.content.luatele == "messageChatDeleteMember" or data.message.content.luatele == "messageChatAddMembers" or data.message.content.luatele == "messagePinMessage" or data.message.content.luatele == "messageChatChangeTitle" or data.message.content.luatele == "messageChatJoinByLink" then
